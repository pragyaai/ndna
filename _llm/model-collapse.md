...
<div class="math">\[\mathcal{T}_{\mathrm{nDNA}} = \left\{ \big( \kappa_\ell, \mathcal{L}_\ell, \| \mathbf{v}_\ell^{(c)} \| \big) \right\}_{\ell=1}^{L}\]</div>
<p>
...
<div class="math">\[\kappa_\ell \to \mathrm{const}, \quad \mathcal{L}_\ell \to \mathrm{min}, \quad \| \mathbf{v}_\ell^{(c)} \| \to \mathrm{uniform}\]</div>
<p>
...
<div class="math">\[\kappa_\ell \leq 0.02, \quad \mathcal{L}_\ell \leq 0.4 \quad \forall \ell > 20\]</div>
<p>contrasted with healthy ranges of:</p>
<div class="math">\[\kappa_\ell \geq 0.05, \quad \mathcal{L}_\ell \geq 0.8\]</div>
...
<div class="math">\[\text{Child}^{(g)} = \operatorname{Merge}\!\left(\text{Child}^{(g-1)},\ \text{Child}^{(g-2)}\right)\]</div>
<p>
...
<p>
  This geometry-inspired framework also leads us toward a <strong>unifying mathematical diagnostic</strong>. If we define the <strong>epistemic vitality function</strong> at layer \(\ell\) as:
</p>
<div class="math">\[\boxed{\mathcal{V}_\ell := \kappa_\ell \cdot \mathcal{L}_\ell \cdot \| \mathbf{v}_\ell^{(c)} \|}\]</div>
<p>then its decay over time:</p>
<div class="math">\[\boxed{\frac{d}{dg} 
\mathcal{V}_\ell^{(g)} < 0}\]</div>
<p>
